...
- (void)sd_setImageWithURL:(NSURL *)url placeholderImage:(UIImage *)placeholder options:(SDWebImageOptions)options progress:(SDWebImageDownloaderProgressBlock)progressBlock completed:(SDWebImageCompletionBlock)completedBlock {
    ...
    if (url) {// 4.A. 如果 url 不为 nil

        // 4.A.1. 判断是否设置 activityIndicator
        if ([self showActivityIndicatorView]) {
            [self addActivityIndicator];
        }

        // 4.A.2 调用 SDWebImageManager 的 -downloadImage... 方法开始加载图片，并获得一个遵循 SDWebImageOperation 协议的 operation
        __weak __typeof(self)wself = self;
        id <SDWebImageOperation> operation = [SDWebImageManager.sharedManager downloadImageWithURL:url options:options progress:progressBlock completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, BOOL finished, NSURL *imageURL) {
            // 4.A.2.1 移除 activityIndicator
            [wself removeActivityIndicator];
            if (!wself) return;
            // 4.A.2.2 宏定义在主线程操作结果
            dispatch_main_sync_safe(^{
                if (!wself) return;
                // 4.A.2.2.1 图片下载成功，并且设置了避免自动设置图片
                if (image && (options & SDWebImageAvoidAutoSetImage) && completedBlock)
                {
                    completedBlock(image, error, cacheType, url);
                    return;
                }
                // 4.A.2.2.2 图片下载成功，自动设置图片
                else if (image) {
                    wself.image = image;
                ...
                // 4.A.2.2.3 图片不存在
                    ...
                    // 4.A.2.2.2.1 如果设置了延迟加载占位图
                    if ((options & SDWebImageDelayPlaceholder)) {
                        wself.image = placeholder;
                        [wself setNeedsLayout];
                    }
                }
                // 4.A.2.3 如果 completedBlock 存在并且 finished，则回调 completedBlock
                if (completedBlock && finished) {
                    completedBlock(image, error, cacheType, url);
                }
            });
        }];
        // 4.A.3 借助 UIView+WebCacheOperation 将获得的 operation 保存到成员变量中去
        [self sd_setImageLoadOperation:operation forKey:@"UIImageViewImageLoad"];
    } else { // 4.B URL 为空时，直接回调 completedBlock，返回错误信息
        dispatch_main_async_safe(^{
