...
/**
 * 在 - (void)sd_setImageWithPreviousCachedImageWithURL: placeholderImage: options: progress:completed:`
 * 这个方法里面就有用到,因为图片的下载是需要时间的,所以这个block回调不止回调一次,会一直持续到图片完全下载或者下载失败才会停止回调

 @param receivedSize 已经接收的图片数据的大小
 @param expectedSize 图片数据的大小
 */
 ...
 * 下载完成的 block
 ...
 @param image 下载完成的图片
 @param data 图片的数据
 @param error 如果有error返回error
 @param finished 以及下载是否完成的BOOl值
 */
typedef void(^SDWebImageDownloaderCompletedBlock)(UIImage *image, NSData *data, NSError *error, BOOL finished);


/**
 设置一个过滤器,为下载图片的HTTP request选取header.最终使用的headers是经过这个block过滤之后的返回值

 @param url 传入的url
 @param headers 传入的headers
 @return 最终使用的headers是经过这个block过滤时候的返回值
 */
 ...
 * 下载完成后是否需要解压缩图片，默认为 YES，解压已经下载缓存起来的图片可以提高性能,但是会消耗大量的内存
 * 默认为YES显示比较高质量的图片,如果你遇到因内存消耗过多而造成崩溃的话可以设置为NO,
 */
@property (assign, nonatomic) BOOL shouldDecompressImages;

/**
 最大的并发下载数，意思是队列中最多同时运行几条线程（默认是6）
 */
@property (assign, nonatomic) NSInteger maxConcurrentDownloads;

/**
 * Shows the current amount of downloads that still need to be downloaded
 * 当前在下载队列的操作总数,readonly(这是一个瞬间值,因为只要一个操作下载完成就会移除下载队列)
 */
